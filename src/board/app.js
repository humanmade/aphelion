...
import { buildSiteTopology, confirmationMatchLabel, DEFAULT_DESKTOP_CONTENT_COLUMNS, desktopContentColumnsForTopology, displayChannel, FULL_FIT_PLACE_LIMIT, groupTopologyChanges, layoutSiteTopology, LEGACY_DESKTOP_CONTENT_COLUMNS, routeContainmentElbows, routeSiteTopologyEdges, siteCardHeight, topologyCameraFrames, topologyRunLabel, visibleTopologyEdges } from '/assets/topology.mjs'
...
const SETTLED_CAMERA_CONTENT_COLUMNS = DEFAULT_DESKTOP_CONTENT_COLUMNS
...
function settledCameraFrame({ compact, aspect, viewportWidth, contentColumns, nodeW, nodeH, gapX, gapY, padX, padY }) {
  if (compact) return {
  ...
  const columns = Math.max(1, contentColumns || SETTLED_CAMERA_CONTENT_COLUMNS)
  ...
  if (columns === SETTLED_CAMERA_CONTENT_COLUMNS) {
    const maximumLegibleWidth = viewportWidth / DEFAULT_CAMERA_LEGIBILITY_FLOOR
    if (width > maximumLegibleWidth) {
      width = maximumLegibleWidth
      height = width / aspect
    }
  }
...
function quantizedCameraFrame(base, nodes, metrics, nodeW, placeCount) {
  const runway = placeCount > SETTLED_CAMERA_PLACE_CAPACITY ? CAMERA_RUNWAY_PADDING : 0
  const right = Math.max(base.x, ...nodes.map(item => item.x + nodeW)) + runway
  const bottom = Math.max(base.y, ...nodes.map(item => item.y + (metrics[item.id]?.h || 104))) + runway
...
function renderComponents(model, topology = currentTopology(model)) {
  ...
  if (siteNodes.length) {
    ...
    if (!state.layoutSeeds.has(layoutSessionId)) state.layoutSeeds.set(layoutSessionId, { desktopWrapColumns: desktopContentColumnsForTopology(topology.topologyVersion) })
    layoutSeed = state.layoutSeeds.get(layoutSessionId)
    const siteLayout = layoutSiteTopology(topology, { compact, nodeW, nodeH: layoutNodeH, nodeHeights: Object.fromEntries(Object.entries(metrics).map(([id, metric]) => [id, metric.h])), gapX, gapY, padX, padY, layoutSeed })
  ...
  if (!state.userMovedCamera) {
    ...
    const settled = settledCameraFrame({ compact, aspect, viewportWidth: flowWidth, contentColumns: layoutSeed?.desktopWrapColumns || LEGACY_DESKTOP_CONTENT_COLUMNS, nodeW, nodeH: layoutNodeH, gapX, gapY, padX, padY })
    if (compact) {
      ...
      const placeCount = displayedNodes.filter(node => node.id !== 'wp:site').length
      const quantized = quantizedCameraFrame(settled, displayedNodes, metrics, nodeW, placeCount)
      if (!state.camera) {
